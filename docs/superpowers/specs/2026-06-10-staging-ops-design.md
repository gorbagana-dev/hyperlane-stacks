...
3. **Keys + funding** — reuses `gen-local-keys.sh`: generates the deployer
   and hot "hardware wallet" keypairs (the hot-key file *is* staging's
   signer per the staging design) and funds the gorchain side from the
   chain's own faucet. The solana side funds via `solana airdrop` on
   devnet; the runbook documents the rate limits and manual top-up
   fallback. No SPL token deploy step.
...
- `secrets.yml`: `cloudflare_api_token`, `helius_api_key` (devnet
  ...
  `ghcr_pat`
- `group_vars`: `HARDWARE_WALLET_PUBKEY` (hot keypair generated by the
  chain play), `IGP_ORACLE_PUBKEY`, `GORCHAIN_VALIDATOR_ADDRESS`,
  `SOLANA_VALIDATOR_ADDRESS` (from Privy)
