...
| D8 | Implementation lands via PRs into `gorbagana` (never directly on it); the operator cuts tags (`vX.Y.Z-gorbagana.N`, GitHub release UI) after the rebase and after the WS work | Tags, not branch tips, are the provenance pins for on-chain programs; tags anchor releases, so the operator creates them manually with the release. Nothing is deployed yet (staging included), so the new base is the first provenance pin for every environment. |
