...
|---|-------|--------|-------------|-------|-------|
| 1 | `hyperlane-svm-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@sealevel-gorbagana-v1.0.0` | `gorbagana-dev/hyperlane-svm-deployer` | — | `hyperlane-svm-deployer` |
| 2 | `hyperlane-svm-warp-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@sealevel-gorbagana-v1.0.0` | `gorbagana-dev/hyperlane-svm-deployer` | — | `hyperlane-svm-warp-deployer` |
...
- The deployer image is built from upstream Hyperlane `16c056a0` (SDK v3.x), mirrored as the fork tag `sealevel-gorbagana-v1.0.0` — its own release line, separate from the agents/scraper (which track `vX.Y.Z-gorbagana.N` and don't build the programs). The `.so` programs are built with plain `cargo build-sbf` → SBPFv0: Solana devnet only enables v0 for deployment (v1/v3 gated off), and gorchain accepts v0 too. Do not add `--arch` (it forces v1/v3, which Solana rejects). Program logic and on-chain account layouts are unchanged, so they remain compatible with the scraper.
...
|---------------|-----------|---------------------------|--------|
| `gorbagana-dev/hyperlane-svm-deployer` | `gorbagana-dev-hyperlane-svm-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@sealevel-gorbagana-v1.0.0` | Multi-stage Rust build of `hyperlane-sealevel-client` + `.so` programs (plain `cargo build-sbf` / SBPFv0) + `solana-verify`. Upstream `16c056a0`, Solana CLI 3.0.14. |
