...
|---|-------|--------|-------------|-------|-------|
| 1 | `hyperlane-svm-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@v2.2.0-gorbagana.3` | `gorbagana-dev/hyperlane-svm-deployer` | — | `hyperlane-svm-deployer` |
| 2 | `hyperlane-svm-warp-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@v2.2.0-gorbagana.3` | `gorbagana-dev/hyperlane-svm-deployer` | — | `hyperlane-svm-warp-deployer` |
...
- The deployer image is built from the gorbagana `hyperlane-monorepo` fork at `v2.2.0-gorbagana.3` (the agents and scraper stay on `.1`; they don't build the programs). `.3` differs from `.1` only by building the `.so` programs with `--arch v0` (SBPFv0) — Solana devnet only enables v0 for deployment (v1/v3 gated off), and gorchain accepts v0 too. Program logic and on-chain account layouts are unchanged, so they remain compatible with the scraper.
...
|---------------|-----------|---------------------------|--------|
| `gorbagana-dev/hyperlane-svm-deployer` | `gorbagana-dev-hyperlane-svm-deployer` | `github.com/gorbagana-dev/hyperlane-monorepo@v2.2.0-gorbagana.3` | Multi-stage Rust build of `hyperlane-sealevel-client` + `.so` programs (built `--arch v0` / SBPFv0) + `solana-verify`. Solana CLI 3.0.14. |
