...
For the deployer: `github.com/gorbagana-dev/hyperlane-monorepo@v2.2.0-gorbagana.4` → `$CERC_REPO_BASE_DIR/hyperlane-monorepo/`.
...
decode different Sealevel account layouts is moot: the deployer and the scraper
both track the gorbagana fork's `vX.Y.Z-gorbagana.N` line (deployer at
`v2.2.0-gorbagana.4`) — same program logic and account layouts.)
